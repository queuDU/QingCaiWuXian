...
double findroute(int destx, int desty,int x,int y, int qdis) {
...
//读取并储存数据
	THUAI4::ColorType mycolor = g.GameApi::GetSelfTeamColor();
	shared_ptr<const THUAI4::Character> myinfo = g.GameApi::GetSelfInfo();
...
#define decorate(x) ((x)<0.0?(2*pi-(x)):(x))
	...
	if(sorted[1].val < 0) { //跑！
		double dir[5]; int cntdir = 1;
		while(sorted[cntdir].val < 0)
